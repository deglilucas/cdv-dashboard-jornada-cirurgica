...
**Status**: implementado e testado nas **três páginas** (Consultas, NPS e Faturamento).
...
- Como as listas saem: a classe **`.print-exclude`** + a regra `display:none !important` dentro do `@media print`. Hoje está em **`cardClinicSummary`, `cardPatientList` e `cardNpsTable`**. Card novo que seja lista/tabela detalhada deve receber essa classe.
